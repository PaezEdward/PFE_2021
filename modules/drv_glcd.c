...
/* delay definition */
#define GLCD_DLY_10US                        800     // 10us
#define GLCD_RESET_DLY                       500     // 5ms
...
static Boolean OddPixel = FALSE;
static Int32U PixelHold = 0;
...
static Int32U XL_Win = DEF_X_LEFT;
static Int32U YU_Win = DEF_Y_UP;
static Int32U XR_Win = DEF_X_RIGHT;
static Int32U YD_Win = DEF_Y_DOWN;
...
pDATCTR_Data_t pDATCTR_Data;
  ...
  // Rest L > 10us H
  GLCD_SetReset(0);
  for(volatile Int32U i = GLCD_DLY_10US; i ; --i);
  GLCD_SetReset(1);
  // Wait 5 ms
...
#error "PARTIAL_MODE not supported yet"
  ...
  if(Glcd_Iss == GLCD_ISS_1)
  {
    // PWRCTR
    Data = 0x1F;
    GLCD_SendCmd(PWRCTR,(pInt8U)&Data,0);
  }
  // Wait for more than 120ms after power control command
	DelayResolution100us(GLCD_BOOSTER_ON_DLY);
  GLCD_SendCmd(GLCD_DISPLAY_STATE?DISPON:DISPOFF,NULL,0);
  return(GLCD_OK);
}

void GLCD_UpdateMemory(pInt8U pInitData)
{
	Int32U Data;
	Int32U i;
