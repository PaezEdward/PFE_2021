...
Int32U i,res, data;
  ...
  for(i=100;i;--i)
  {
    ...
    // SPP +
    // datasheet: Production Manual SDCard v1.9, chapter 3.11 page 37:
    // "If bit 32 (the busy bit) is set, it informs the host 
    // that the card power up procedure is finished."
    // In this case we don't have to check the returned res.
    if (!(data & 0x80000000))
    // SPP -
      if (~_SD_ILDE_STATE & res)
      {
...
Int32U res,i;
  ...
  {
    res = SdTranserByte(0xFF);
    res = SdTranserByte(_SD_DATA_TOLKEN);
    // Send block
...
void SdStatusUpdate (void)
{
  ...
  if((DiskCommandPass != _SdDskCtrlBlk.DiskStatus))
  {
    ...
    case _SdCardError:
      ...
      break;
    case _SdNoResponse:
      _SdDskCtrlBlk.DiskStatus = DiskUknowError;
    break;
    default:
      _SdDskCtrlBlk.DiskStatus = DiskNotPresent;
      break;
