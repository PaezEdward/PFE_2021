  ...
  while ( ep_intr )
  {
    if (ep_intr&0x1) /* In ITR */
    {
      ...
      if ( diepint.b.xfercompl )
      {
        ...
        {
          OTG_DEV_SetEPTxStatus(epnum, DEV_EP_TX_NAK);
    ...
    if (ep_intr&0x1)
    {
      ...
      if ( doepint.b.setup )
      {
        if (epnum == 0)
        {
          ...
          //OTGD_FS_EP0StartXfer(ep); // SBB
          USBF_EP_DoSetupPkt((void *)ep->xfer_buff);
